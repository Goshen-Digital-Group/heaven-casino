...
const neonPulse = keyframes`
  0% {
    box-shadow: 0 0 32px #fff459ff, 0 0 64px #ffd70044;
    border-color: #ffd70044;
  }
  50% {
    box-shadow: 0 0 48px #ffd700, 0 0 96px #fcff5988;
    border-color: #ffd700;
  }
  100% {
    box-shadow: 0 0 32px #fffc59ff, 0 0 64px #ffd70044;
    border-color: #ffd70044;
  }
`;
...
const Banner = styled.div`
  ...
  overflow: hidden;
  aspect-ratio: 15 / 5;
  background: linear-gradient(180deg, rgba(231, 235, 215, 0.8) 0%, #3f3942ff 80%, #070607ff 100%);
  backdrop-filter: blur(20px);
  -webkit-backdrop-filter: blur(20px);
  box-shadow: 0 0 32px #eeff59ff, 0 0 64px #ffd70044;
...
const Heading = styled.h2`
  ...
  font-weight: 700;
  text-align: center;
  text-shadow: 0 0 16px #ffd700, 0 0 32px #fff459ff;
  font-family: 'Luckiest Guy', cursive, sans-serif;
  letter-spacing: clamp(0.5px, 0.5vw, 2.5px);
  margin-bottom: 0.5rem;
  &::after {
    content: " — Every Spin, A Surge of Hope.";
    font-weight: 400;
...
function useRandomQuote(): string {
  ...
  return `"${QUOTES[index]}" #HeavenlyFather`;
}
...
export function WelcomeBanner() {
  ...
  return (
    <Container $isVisible={shouldShow} $isLoading={isLoading}>
      <Banner>
        <HeroOverlay>
          <Heading>Welcome to Heavenly Casino</Heading>
        </HeroOverlay>
        <BannerBottomBar>
